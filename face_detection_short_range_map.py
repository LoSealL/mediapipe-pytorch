...
with open('model/face_detection_short_range.tflite', 'rb') as fd:
...
# open pytorch model
model_fd = FaceDetection()
state = model_fd.state_dict()

# make a list of weights in tf lite model
k = 0
with open('model/face_detection_short_range.map', 'w') as fd:
    weights_tflite = []
...
with open('model/face_detection_short_range.map', 'a') as fd:
    fd.writelines(["\n"])
...
inter = tf.lite.Interpreter('model/face_detection_short_range.tflite')
...
torch.save(state, "model/face_detection_short_range.pth")
try:
    model_fd.load_state_dict(state, strict=True)
    torch.onnx.export(model_fd, (torch.randn(1, 3, 128, 128),), 'model/face_detection_short_range.onnx',
