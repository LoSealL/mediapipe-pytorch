...
def is_supported_instance(module):
    if type(module) in MODULES_MAPPING or type(module) in CUSTOM_MODULES_MAPPING:
        return True
    for abc in MODULES_MAPPING:
        if issubclass(type(module), abc):
            CUSTOM_MODULES_MAPPING.update({type(module): MODULES_MAPPING[abc]})
            return is_supported_instance(module)
    return False
